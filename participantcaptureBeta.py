...
class webcamThread(Thread):
    def __init__(self, webcamNumber, folderNames):
        Thread.__init__(self)
        self.webcamNumber = webcamNumber
        self.folderNames = folderNames
    def run(self):
        camCapture(self.webcamNumber, self.folderNames)
def camCapture(webcamNumber, folderNames):
    ...
    while webcam.isOpened():
        ...
        if ret is True:
            ...
            cv2.imwrite(folderNames[0] +'webcam' + str(webcamNumber+1) + '/' + imgName, frame)
...
def capture(scenerioNumber, folderNames, currentFolder):
    global img
        ...
        while(1):
            with renderer.frame_condition:
                if renderer.frame_condition.wait(150.0 / 1000.0):
                    ...
                    file = open(folderNames[0] + '/seek_temperatures/' + str(capTime) + '.csv', 'w')
                    ...
                    cv2.imwrite(folderNames + '/seek_jpg/' + imgName, imgt_rescaled)
